...
public:
    ...
    // 被认为是外点的特征点置ｔｒｕｅ，默认值是ｆａｌｓｅ.根据关键帧来初始化位姿，然后再根据与关键帧的匹配来优化位姿的过程中可能会被置ｔｒｕｅ,用于标记某个点为外点，具体参考TrackReferenceKeyFrame（）函数.被认为是外点的地图点会跟该当前帧对应的特征解除关联。用完他是外点的信息后最终会置ｆｌａｓｅ,
    //此外，在得到当前帧的位姿后，进行局部地图跟踪过程中会根据局部地图与当前帧的匹配关系进行位姿优化，被认为是外点的特征点置ｔｒｕｅ。这次内外点信息使用后没有被置为ｆａｌｓｅ,因为绘图线程需要使用。
