...
// 'l'表示list数据类型
// "KF"表示KeyPoint数据类型
//system类的构造函数中会声明ｔｒａｃｋing类，ｌｏｃａｌｍａｐｐｉｎｇ类和回环类。因此Ｔｒａｃｋｉｎｇ类构造函数的参数直接来自ｓｙｓｔｅｍ类的构造函数。
namespace ORB_SLAM2
{
...
// 输入左目RGB或RGBA图像和深度图，时间戳，返回当前帧的位姿
...
void Tracking::Track()
{
	...
	//跟踪局部地图
	    ...
	    //mbVO为真时，我们不使用局部地图，也就不需要跟踪局部地图。一旦系统重定位后，我们需要会再次需要局部地图
            // 重定位成功
            if(bOK && !mbVO)
                bOK = TrackLocalMap();
        }
	//对于定位建图模式来说，跟踪局部地图成功的话，系统状态保存为ok,跟踪失败，系统状态变为ｌｏｓｔ
        if(bOK)
            mState = OK;
        else
            mState=LOST;
	...
	//跟踪局部地图成功后，更新运动模型，清除临时地图点，判断是否需要插入新的关键帧（需要的话就用当前帧来生成关键帧）
        if(bOK)
        {
            // Update motion model
            if(!mLastFrame.mTcw.empty())//上一帧的位姿不为空时，填充运动模型
            {
            ...
            {
                if(mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvbOutlier[i])
                    mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);//对于双目，这一步在跟踪局部地图的时候已经完成
            }
        }
 ...
 */
//mpInitializer单目初始器最初是没有没初始化的，第一帧图像到达后构造ｆｒａｍｅ,实现关键点的提取和描述子计算，然后进入到ｔｒａｃｋ()函数
//由于mpInitializer还没有初始化，先判断第一帧提取的关键点数量是否大于１００，不是的话ｔｒａｃｋ()函数结束，重新接受图像图。若大于１００，则把该帧设为初始化第一帧，也就四ｓｌａｍ逻辑的第一帧。
//并用第一帧来初始化单目初始器mpInitializer。然后ｔｒａｃｋ()函数结束，准备接受第二帧图像
//用第二帧图像生成ｆｒａｍｅ当前帧，它的关键点也要求大于１００，否则删除初始器，重新初始化。
//有了初始化的两帧后就可以用特征匹配类里的SearchForInitialization（）函数来寻找初始化两帧之间的匹配关系。
//若匹配的点对少于１００对，则重新初始化。大于１００的话，就用初始器通过H模型或F模型进行单目初始化，得到两帧间相对运动、初始MapPoints
//并把生成的地图点注册到地图和关键帧中。初始化完成后系统状态变成mState：ｏｋ.
...
void Tracking::MonocularInitialization()
{
    ...
    {
        // Try to initialize
        // 步骤2：如果当前帧特征点数大于100，则得到用于单目初始化的第二帧(之前已经有一帧mInitialFrame)
        // 如果当前帧特征点太少，重新构造初始器
 ...
 * 2. 对属于同一node的描述子进行匹配
 * 3. 根据匹配对估计当前帧的姿态(BA优化，只优化当前帧的位姿，不优化地图点)
 * 4. 根据姿态剔除误匹配
 ...
 * 可以通过深度值产生一些新的MapPoints//临时的，跟踪完后需要删除，而且没有为这些地图添加属性，如能观测到它们的帧，更新法向量和深度等操作，这些操作是需要添加到地图中的地图点被创建后必须马上进行的
 */
 ...
 * 2. 对局部MapPoints进行投影匹配，为当前帧匹配更多的地图点
 * 3. 根据匹配对优化当前帧的姿态
 * 4. 根据姿态剔除误匹配（根据优化后的内外点信息剔除地图点与当前帧的无匹配）
...
bool Tracking::TrackLocalMap()
{
    ...
    Optimizer::PoseOptimization(&mCurrentFrame);//这个优化只优化位姿，不优化与当前帧关联的局部地图点。同时在优化后某些与当前帧关联的地图点会被标记为外点或者内点。
    mnMatchesInliers = 0;
 ...
 */
//该函数更新了当前帧与局部地图的匹配关系，具体体现在更新了当前帧的mvpMapPoints。
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    map<KeyFrame*,int> keyframeCounter;//与当前帧共视的关键帧和权重
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
