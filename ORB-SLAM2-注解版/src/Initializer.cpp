                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend; i++)
    {
        if(vMatches12[i]>=0)
        {
            mvMatches12.push_back(make_pair(i,vMatches12[i]));//每一个元素是点对
            mvbMatched1[i]=true;
        }
        else
            mvbMatched1[i]=false;
    ...
    // mMaxIterations:200
    //mvsets里储存了２００组把对点的索引，通过索引在mvMatches12就可以找到对应的点对
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // 恢复原始的均值和尺度
	//p2'=H21'p1',而H21'=T2*H21*H1^(-1).这个公示直接把p1',p2'用p1,p2表示即可推出
        H21i = T2inv*Hn*T1;
        H12i = H21i.inv();
...
{
    // Number of putative matches
    //const int N = vbMatchesInliers.size();///????错误，改前mvMatches12
    const int N = mvMatches12.size();
 ...
 */
//对Ａ矩阵进行奇异值分解，最小奇异值对应的特征值向量就是要求的Ｈ矩阵。A=uwvT,w为对角矩阵，特征值递减排列，ｖ的每一列为对应特征向量，但是ｃｖ输出的为ｖＴ,故对应为行
...
{   
    ...
    for(int i=0; i<N; i++)
    {
        ...
        // |v1| = s|h21inv h22inv h23inv||v2|
        // |1 |    |h31inv h32inv h33inv||1 |
        const float w2in1inv = 1.0/(h31inv*u2+h32inv*v2+h33inv);//s
                      ...
                      cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    ...
    float x1[] = {aux1,aux1,-aux1,-aux1};//列表初始化
 ...
 */
//对好的匹配点进行三角化，得到３Ｄ点，检查它的深度值，视差值（在两帧中视角的夹角）、计算重投影误差。函数返回的是满足要求的点数
//vbgood是满足要求的点且视差较大的点对应的变量置ｔｒｕｅ.
//所有满足要求的点中的较大视差
