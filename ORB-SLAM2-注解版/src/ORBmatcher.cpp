 ...
 * @param  th          阈值//用于将搜索特征点的半径进行放大
 * @return             成功匹配的数量
 * @see SearchLocalPoints() isInFrustum()
 */
//参数vpMapPoints里的局部地图点都是可以投影到当前帧的点，它们是先通过SearchLocalPoints()函数筛选出来的地图点，再把筛选出来的地图点通过isInFrustum()计算它们在当前帧中的一些跟踪参数
//遍历每一个地图点，寻找匹配，对每一个地图点，步骤１：先判断该地图点是否需要投影到当前帧（mbTrackInView），再判断该地图点是是坏点，上述不成立则退出该次循环，满足则读取该点的预测的金字塔层数（相对于该帧的）
//步骤２：根据该点在当前帧中被跟踪的视角范围，初步确定搜索半径（通过RadiusByViewingCos（）函数实现），再将初步确定的半径乘以ｔｈ，得到最终的搜索半径ｒ.
//步骤３：根据地图点在当前的投影位置，以r为半径，取出这个区域内的特征点，这是通过调用函数Ｆｒａｍｅ::GetFeaturesInArea（）函数实现的，若该区域没有特征点，则该次循环结束。
//步骤４：取出该地图点的代表描述子，将它与第三步取出的所有特征点计算距离，记录最近距离，最近距离对应的特征索引bestIdx，以及该特征点所在的金字塔层数；同时第二近的距离，对应的特征索引，以及该特征点所在的金字塔层数
//步骤５：最小距离要求小于最大的距离阈值，否则该次循环结束。若满足则继续。当第一近与第二近的特征点取自同一层时，需要计算计算他们的比值，若不小于给定的比值阈值，则没有找到匹配点，该次循环结束。若它们属于不同的金字塔层，则认为最小距离对应的特征点为地图点的匹配点。
//将该地图点存到mvpMapPoints变量对应的位置中bestIdx。同时将nmatches加１，表明配对数加１。
//上面只是初步匹配，后面应该还有对匹配筛选的过程
//tracking线程的局部地图追踪进行位姿估计
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)
{
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        if(bestDist<=TH_HIGH)//距离阈值
        {
            if(bestLevel==bestLevel2 && bestDist>mfNNratio*bestDist2)//比例阈值
                continue;
...
//根据地图点的法线与在该帧中的观测方向的夹角来确定搜索的半径。点的法线是该点第一次被创建是，该点在世界坐标系的坐标与相机光心在世界坐标连线的单位向量。
//而观察方向为该点在世界坐标系的坐标与当前帧光心在世界坐标系下的连线。
...
//检查极线距离
//已知基础矩阵为Ｆ１２（２相对与１）,两幅图像的点为p2,p1,由对极约束可知p2'F21p1=0
 ...
 * @return                   成功匹配的数量
 */
//关键帧和当前帧通过词袋进行快速匹配（1.用在tracking线程根据参考关键帧进行线程追踪过程中2.重定位)
//vpMapPointMatches里储存普通帧与关键帧的匹配，索引对应普通帧关键点，里面的地图点代表关键帧对应的地图点
int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
{
    ...
    while(KFit != KFend && Fit != Fend)
    {
        if(KFit->first == Fit->first) //步骤1：分别取出属于同一node的ORB特征点(只有属于同一node，才有可能是匹配点)
        {
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                if(bestDist1<=TH_LOW) // 匹配距离（误差）小于阈值//距离阈值
                {
                    // trick!
    ...
    // 根据方向剔除误匹配的点
    //所有匹配的特征点的角度偏差应该在同样的范围内，这里放宽了限制，在三个范围内也认为是正确的匹配。（匹配点的角度偏差应该满足两帧之间的旋转变化的角度，轴向量）
    if(mbCheckOrientation)//距离阈值
    {
        int ind1=-1;
        int ind2=-1;
        int ind3=-1;
...
//该函数用于回环检测线程里，关键帧位姿已知。相似变换矩阵（在回环检测过程中通过sim求解器求解）。回环检测线程中计算得到相似矩阵之后匹配回环关键帧（回环末端）和当前关键帧（回环始帧）的局部地图点进行匹配
//vpPoints是回环帧里待匹配的地图点，vpMatched里储存的关键帧ｐＫＦ里已经与回环帧地图点匹配的的特征，没有匹配的特帧对应的内容里为空指针
//步骤１：先将世界坐标系相对于关键帧的相似变换转换成投影变换，来同一尺度。在遍历所有的vpPoints里的点（即候选关键帧里的所有地图点），下面针对每个地图点做操作
//步骤２：判断地图点是否为坏点或者地图点是否已经匹配到当前关键帧，若是坏点或者已经匹配，该次循环结束
//步骤３：将该地图点投影到当前关键帧中，先判断深度是否为正，否的就结束该次循环。在判断投影的像素点是否在该关键帧里（调用函数pKF->IsInImage(u,v)完成），不在的话也结束
//步骤４：计算该点在关键帧坐标系下到相机光心距离，判断是否在最大最小距离内，不在则结束该次循环。判断该点在本帧的视角，大于６０度，则结束。再计算该点在该关键帧中的预测尺度，根据尺度来确定搜索半径。
//步骤５：根据投影像素点位置，搜索半径取出搜索区域里的所有特征点，遍历所有特征点，计算特征点与地图点描述子之间的最小距离，若距离满足距离阈值，则认为匹配上。将地图点储存到vpMatched的对应位置。
int ORBmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched, int th)
{
    ...
    cv::Mat Rcw = sRcw/scw;
    cv::Mat tcw = Scw.rowRange(0,3).col(3)/scw;// 相机坐标系下，世界坐标系的坐标
    cv::Mat Ow = -Rcw.t()*tcw;// 世界坐标系下相机光心的坐标
...
//vbPrevMatched为预测的Ｆ１里的关键点在Ｆ２里的像素坐标的集合，但是怎么来的不太明白？？？
//这个函数子找到匹配后用F1的匹配点的坐标（即对应的Ｆ２里的坐标）来更新vbPrevMatched
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        for(vector<size_t>::iterator vit=vIndices2.begin(); vit!=vIndices2.end(); vit++)
        {
            ...
            if(vMatchedDistance[i2]<=dist)//部分实现双向匹配
                continue;
        ...
        if(bestDist<=TH_LOW)
        {
            ...
            {
                if(vnMatches21[bestIdx2]>=0)//部分实现双向匹配
                {
 ...
 */
//3d-3d的匹配
//pKF1与pkf2匹配，vpMatches12里存放pKF2中的地图点，对应索引为关键帧１里的特征索引
 ...
 * @param pKF2          关键帧2
 * @param F12           基础矩阵
 * @param vMatchedPairs 存储匹配特征点对，特征点用其在关键帧中的索引表示（后面用于三角化）
...
/**
 * @brief 先判断该地图点是否已经是关键帧的地图点（pMP->IsInKeyFrame(pKF)）根据函数，是的话就必要进行了。然后将MapPoints投影到关键帧pKF中，并判断是否有重复的MapPoints
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    for(int i=0; i<nMPs; i++)
    {
        ...
        if(pMP->isBad() || pMP->IsInKeyFrame(pKF))//如果该地图点是ｂａｄ或者已经在相邻关键帧中了，则无需融合
            continue;
...
//在将（终端）回环地图点与当前关键帧（起始端）地图点融合的过程中，如果（终端）回环地图点要与当前地图点融合，则先不融合而是先把它们储存到vpReplacePoint中。ｉｄｘ对应vpPoints的ｉｄｘ,提地图点对应当前帧的地图点。如果vpReplacePointd的ｉｄｘ里没有地图点，说明vpPoints里对应点不用融合
...
//先将ＫＦ1对应有地图点且没有匹配到ＫＦ２的地图点根据相似变换投影到ＫＦ２中，根据预测的金字塔层数确定搜索半径，得到匹配关系储存在vnMatch1，vnMatch1的索引对应ＫＦ1的特征点，里面储存的是对应的ＫＦ２的索引。这个匹配只使用了距离阈值，而且较为宽松
//同理找到ＫF2与ＫF1的匹配关系，储存在vnMatch２.
//满足双向匹配的匹配关系被接受。最后将所有的匹配关系都储存到vpMatches12中
 ...
 * @see SearchByBoW()
 */
//用于根据运动模型寻找匹配估计相邻两帧位姿
//按照运动模型估计的位姿将上一帧的地图点投影到当前帧，来估计位姿。除了利用当前帧相对上一帧是前进还是后退的信息来确定尺度的范围内，跟前面提到的匹配基本一样
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
{
    ...
    const cv::Mat tlc = Rlw*twc+tlw; // Rlw*twc(w) = twc(l), twc(l) + tlw(l) = tlc(l)//当前帧坐标系原点在上一帧坐标系下的坐标
    ...
    for(int i=0; i<LastFrame.N; i++)
    {
		...
		//对于非单目的情况下，当前帧相对上一帧前进，则对应的尺度应该增大，因此限制最小尺度；相对前一帧后退，尺度应该减小，因此限制最大尺度
		//对于单目的情况下，特征匹配在在相邻的层找
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, KeyFrame *pKF, const set<MapPoint*> &sAlreadyFound, const float th , const int ORBdist)
{
    ...
    const cv::Mat Rcw = CurrentFrame.mTcw.rowRange(0,3).colRange(0,3);
    const cv::Mat tcw = CurrentFrame.mTcw.rowRange(0,3).col(3);
    const cv::Mat Ow = -Rcw.t()*tcw;//当前帧在世界坐标系下的坐标
...
// 取出直方图中值最大的三个index,histo是两帧之间匹配特征点的偏转角的统计直方图，将整个偏转角划分为３０个不同的档次，但是按理说两张图之间的旋转只有一个，只需要取最大的来表示两图之间的旋转范围即可，但是考虑到对应特征点附近的亮度变化，我们取数量最大的前三个档次
//若数量最大的偏转角档次占有绝对优势，则可认为两帧之间的偏转角就在该档次里，匹配点之间的偏转角角度不满足这个档次范围的可认为是误匹配
...
int ORBmatcher::DescriptorDistance(const cv::Mat &a, const cv::Mat &b)
{
    ...
    {
        unsigned  int v = *pa ^ *pb;//^按位异或;&按位与；>>右移
        v = v - ((v >> 1) & 0x55555555);
