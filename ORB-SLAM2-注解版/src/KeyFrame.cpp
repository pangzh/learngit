...
void KeyFrame::ComputeBoW()
{
	...
	//mpORBvocabulary->transform()根据图像特征的描述子将图像转换成词袋向量mBowVec和特征值向量mFeatVec（正向索引），这里的特征向量描述了图像特征与第四层节点之间的关联关系，用来加速匹配
...
void KeyFrame::SetPose(const cv::Mat &Tcw_)
{
    ...
    Ow.copyTo(Twc.rowRange(0,3).col(3));	//相机中心在世界坐标系下的坐标
    ...
    Cw = Twc*center;	//立体相机中心在世界坐标系下的坐标
}
 ...
 */
//mConnectedKeyFrameWeights变量存储的是与当前帧有共视关系的其他关键帧及其共视的权重
 ...
 */
//（４）将mConnectedKeyFrameWeights里的关键帧和权重分别存储到mvpOrderedConnectedKeyFrames和mvOrderedWeights中，
//并且按权重从大到小排序
void KeyFrame::UpdateBestCovisibles()
{
    ...
    // 按照权重进行排序
    sort(vPairs.begin(),vPairs.end());//按权重从小到大排序
    list<KeyFrame*> lKFs; // keyframe
    list<int> lWs; // weight
    for(size_t i=0, iend=vPairs.size(); i<iend;i++)
    {
        lKFs.push_front(vPairs[i].second);//将权重大的插到前面
...
vector<KeyFrame*> KeyFrame::GetCovisiblesByWeight(const int &w)
{
    ...
    // 在从小到大排序的数组中，lower_bound是返回小于等于，而upper_bound只能返回第一个大于的
    //但是在从大到小的排序数组中，重载了ｌｏｗｅｒ_bound和ｕｐｐｅｒ_bound.
...
//根据地图点在关键帧中的索引，删除该帧中与特征ｉｄｘ有关联的地图点关联，
void KeyFrame::EraseMapPointMatch(const size_t &idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvpMapPoints[idx]=static_cast<MapPoint*>(NULL);
}
//根据地图点来删除该关键帧与该地图点的索引
...
//更新与该关键帧特征ｉｄｘ有关联的地图点，用pMP来替换原来的地图点
void KeyFrame::ReplaceMapPointMatch(const size_t &idx, MapPoint* pMP)
{
    mvpMapPoints[idx]=pMP;
}

//返回所有的与该关键帧有关联的好的地图点，好的地图点是指该地图点能被三个及以上的关键帧看到
 ...
 * @param  minObs 最小观测
 */
//该帧中被跟踪的地图点数量，这些被跟踪的地图点首先是好的，而且被关键帧观察到的数量不少于minObs
...
//根据特征索引返回与其关联的地图点，即返回mvpMapPoints[idx]。
...
void KeyFrame::UpdateConnections()
{
    ...
    sort(vPairs.begin(),vPairs.end());//不写第三个参数，默认按从小到大排序
    list<KeyFrame*> lKFs;
    list<int> lWs;
    for(size_t i=0; i<vPairs.size();i++)
    {
        lKFs.push_front(vPairs[i].second);//大的插到前面，按从大到小排序
        lWs.push_front(vPairs[i].first);
    }
...
//将参数列表里的关键帧添加到子关键帧的集合中
void KeyFrame::AddChild(KeyFrame *pKF)
{
    unique_lock<mutex> lockCon(mMutexConnections);
    mspChildrens.insert(pKF);
}
//将参数列表里的关键帧从子关键帧集合里剔除
...
//用参数列表里的关键帧来替换父关键帧，同时更新父关键帧的子关键帧
...
//查看当前关键帧是否含有子关键帧pKF（参数列表的帧），有则返回ｔｒｕｅ,没有返回ｆａｌｓｅ
...
//添加当前帧与参数列表里的关键帧之间的回环边，mspLoopEdges里保存的是与当前关键帧有回环边的关键帧
...
//返回回环边的集合，即返回回环边对应的关键帧的集合mspLoopEdges
...
void KeyFrame::SetErase()
{
    ...
    // 这个地方是不是应该：(!mbToBeErased)，(wubo???)//这个地方就应该是mbToBeErased。所有关键帧的剔除都是SetBadFlag()彻底完成才实现的。
    //原因：SetErase()只用在回环线程中。在对某一关键帧就行回环检测时，为了避免该关键帧被关键帧剔除策略剔除，回环线程会先调用SetNotErase()将mbNotErase置ｔｒｕｅ.
    //在mbNotErase被置ｔｒｕｅ后，若该关键帧被关键帧剔除策略认定为剔除，则局部建图线程里会调用该关键帧的SetBadFlag()来实现剔除，程序运行到if(mbNotErase)后会将mbToBeErased变量置ｔｒｕｅ,然后SetBadFlag()结束，该关键帧并没有剔除。但是该帧是应该被剔除的，应为它是冗余的，所以mbToBeErased表示该帧稍微再剔除。这个稍后剔除的功能还是有回环线程实现的。
    //在完成对该帧的回环检测后，若该帧没有检测到回环，则回环线程让该关键帧调用SetErase()，由于mbToBeErased是ｔｒｕｅ,于是进入到判断语句中，开始该关键帧开始执行SetBadFlag()，这次该函数可以顺利执行完，因为mbNotErase已经被置为ｆａｌｓｅ.当然，若该关键帧检测到了回环，则该帧不会被剔除。因为mbNotErase始终为真。
    //综上分析可知，mbToBeErased的设置对整个ｓｌａｍ系统影响不是很大，按照ｗｕｂｏ的设法，会将一些本该剔除的冗余关键帧保留下来，增加了冗余信息，但对精度没有影响。
...
//删除与关键帧pKF的连接，该操作是通过删除调用该函数的关键帧里的mConnectedKeyFrameWeights变量里的帧pKF来实现，删除后，调用帧需要更新共视信息。
