     ...
     mpReferenceKF(static_cast<KeyFrame*>(NULL))
{
    ...
    // ORB extraction
    // 同时对左右目提特征
    //// 提取的关键点存放在mvKeys和mvKeysRight,描述子存放在mDescriptors和mDescriptorsRight中
...
{
    //调用ORBextractor类里的重载过的（）来提取图像的关键点和描述子
    if(flag==0)
 ...
 */
//判断在该帧下的深度是否为正，投影是否子啊图像内，距离是否子啊最大最小范围内，视角是否足够小。满足这些条件后就可以为该点添加被当前帧跟踪的属性了
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
    ...
    // 标记该点将来要被投影
    pMP->mbTrackInView = true;//说明该点可以被跟踪
    pMP->mTrackProjX = u;
 ...
 * @param y        图像坐标v
 * @param r        边长
 * @param minLevel 最小尺度,特征点所在的金字塔尺度
 * @param maxLevel 最大尺度
 * @return         满足条件的特征点的序号
 */
...
{
    //用－１来初始化ｍｖｕRight,mvDepth
    mvuRight = vector<float>(N,-1.0f);
    mvDepth = vector<float>(N,-1.0f);

    const int thOrbDist = (ORBmatcher::TH_HIGH+ORBmatcher::TH_LOW)/2;

    //获取第０层金字塔中图像的行数，即原始图像的行数
    ...
    for(int i=0; i<nRows; i++)
        vRowIndices[i].reserve(200);	///<每一行最多装２００个特征点
    ...
    for(int iR=0; iR<Nr; iR++)
    {
        ...
        const float r = 2.0f*mvScaleFactors[mvKeysRight[iR].octave];	///<根据关键点所在金字塔的层数来确定搜索的半径
        const int maxr = ceil(kpY+r);
        const int minr = floor(kpY-r);

        for(int yi=minr;yi<=maxr;yi++)
            vRowIndices[yi].push_back(iR);	//我们认为从minr到ｍａｘｒ这多行里都有ｉＲ对应的特征点特征点
    }
    ...
    for(int iL=0; iL<N; iL++)
    {
        ...
        for(size_t iC=0; iC<vCandidates.size(); iC++)
        {
            ...
            // 仅对近邻尺度的特征点进行匹配,左右靓图在ｚ方向没有前后关系，不用类似与后面的考虑运动模型时的相邻帧前进后退对尺度产生增大缩小约束，因此直接取临近尺度。
        ...
        if(bestDist<thOrbDist)
        {
            ...
            for(int incR=-L; incR<=+L; incR++)
            {
                ...
                vDists[L+incR] = dist; // 正常情况下，这里面的数据应该以抛物线形式变化,开口向上，
            }
