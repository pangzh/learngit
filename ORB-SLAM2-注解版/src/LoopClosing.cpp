...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        {
            // Detect loop candidates and check covisibility consistency//检测回环候选者，并进行一致性检测
            if(DetectLoop())
            {
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        for(size_t iG=0, iendG=mvConsistentGroups.size(); iG<iendG; iG++)
        {
            ...
            if(bConsistent)
            {
                ...
                //这里是不是缺一个break来提高效率呢？(wubo???)//同意加ｂｒｅａｋ,因为一旦确定当前的子候选组属于其中一个子连续组，就说明该子候选组是连续的了，没必要再遍历其他的子连续组
            }
        }
...
bool LoopClosing::ComputeSim3()
{
    ...
    for(int i=0; i<nInitialCandidates; i++)
    {
        ...
        int nmatches = matcher.SearchByBoW(mpCurrentKF,pKF,vvpMapPointMatches[i]);//3d-3d
