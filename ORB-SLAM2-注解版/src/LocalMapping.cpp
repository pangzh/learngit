...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            // 从待处理的关键帧序列中取出一帧计算关键帧特征点的BoW映射，把当前帧的地图点关联到当前关键帧，将关键帧插入地图
            ProcessNewKeyFrame();

            // Check recent MapPoints
            // VI-B recent map points culling
            // 剔除ProcessNewKeyFrame函数中引入的不合格MapPoints
            MapPointCulling();//该函数不剔除初始化过程生成的地图点
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    // 但没有对这些匹配上的MapPoints与当前帧进行关联,即没有将该帧添加到该地图点的观测帧中pNewMP->AddObservation(pKF,i);pNewMP->ComputeDistinctiveDescriptors();pNewMP->UpdateNormalAndDepth();等操作没有进行
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            if(cosParallaxRays<cosParallaxStereo && cosParallaxRays>0 && (bStereo1 || bStereo2 || cosParallaxRays<0.9998))
            {
                ...
                x3D = x3D.rowRange(0,3)/x3D.at<float>(3);//把其次坐标变为非齐次坐标
            }
