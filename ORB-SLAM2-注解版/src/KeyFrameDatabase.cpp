...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F)
{
    ...
    // 步骤1：找出和当前帧具有公共单词的所有关键帧
    {
        ...
        for(DBoW2::BowVector::const_iterator vit=F->mBowVec.begin(), vend=F->mBowVec.end(); vit != vend; vit++)
        {
            ...
            for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)
            {
                ...
                {
                    pKFi->mnRelocWords=0;//储存该重定位获选帧与重定位帧之间的相同节点数，第一次使用时需要先清零，因为之前的重定位可能已经被给它赋过值
                    pKFi->mnRelocQuery=F->mnId;//标记关键该帧是重定位在帧的候选中
                    lKFsSharingWords.push_back(pKFi);
                }
                pKFi->mnRelocWords++;
            }
    ...
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        if((*lit)->mnRelocWords>maxCommonWords)//统计具有的最大相同ｗｏｒｄ数,通过它来确定阈值
    ...
    // 步骤3：遍历所有闭环候选帧，挑选出共有单词数大于阈值minCommonWords且单词匹配度大于minScore（这部分貌似没有写，实现也很简单，仿造上面的代码几率最大得分再筛选即可）存入lScoreAndMatch
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;
    ...
    list<pair<float,KeyFrame*> > lAccScoreAndMatch;
    float bestAccScore = 0;//所有组中的最好累加得分
    ...
    for(list<pair<float,KeyFrame*> >::iterator it=lScoreAndMatch.begin(), itend=lScoreAndMatch.end(); it!=itend; it++)
    {
        ...
        for(vector<KeyFrame*>::iterator vit=vpNeighs.begin(), vend=vpNeighs.end(); vit!=vend; vit++)
        {
            ...
            accScore+=pKF2->mRelocScore;// 只有pKF2也在闭环候选帧中，才能贡献分数//这里好像有问题，并不是每一个候选闭环帧都有得分的，只有相同ｗｏｒｄ数大于０．８倍的最大相同ｗｏｒｄ数的帧才计算了。因此其他帧的得分可能是之前重定位的时候算的，因此感觉才前面筛选候选定位帧的时候可以把这个变量置０
